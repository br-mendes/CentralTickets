...
async function initSession(): Promise<string> {
  if (!INST.BASE_URL) throw new Error('GLPI_PETA_URL não configurado')
...
function processRows(rows: any[]): TD[] {
  return rows.flatMap(r => {
    ...
    const cat = norm(r[7] || '') || 'Não categorizado'
    return [{
      ticket_id:           tid,
      instance:            INST_NAME,
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (mode === 'full') {
      ...
      console.log(startPage > 0 ? `[${INST_NAME}] retomando página ${startPage}` : `[${INST_NAME}] full sync início`)
