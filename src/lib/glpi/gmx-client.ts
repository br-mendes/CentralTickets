export class GmxClient {
  ...
  async initSession(): Promise<string> {
    ...
    if (!second.res.ok) throw new Error(this.formatError('GMX initSession failed', second.res, second.text))
    throw new Error('GMX initSession missing session_token | body=' + (second.text || first.text || '<empty body>'))
  }
  ...
  async searchTickets(sessionToken: string, limit = 100): Promise<any[]> {
    ...
    if (!res.ok) throw new Error(this.formatError('GMX searchTickets failed', res, text))
