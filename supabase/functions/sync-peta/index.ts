...
const INST = {
  BASE_URL:   Deno.env.get('NEXT_PUBLIC_GLPI_PETA') ?? '',
  USER_TOKEN: Deno.env.get('PETA_USER_TOKEN') ?? '',
  APP_TOKEN:  Deno.env.get('PETA_APP_TOKEN')  ?? '',
}
...
async function initSession(): Promise<string> {
  if (!INST.BASE_URL) throw new Error('GLPI_PETA_URL n├úo configurado')
